...
(define (back-ground)
  (let* ((tower (rectangle  15 300 "solid" "Burlywood"))
         (movebg (place-image (rectangle 1000 90 "solid" "Forest Green")500 455 (bground)))
         (moov (place-image (text "MOVES : " 36  "white") 200 455 movebg))
         (rec (rectangle 250 50 "solid" "blue"))
         (p1 (place-image rec 500 455 moov))      
         (restart (place-image (text "RESTART ↺" 36  "white") 500 455 p1))
         (p2 (place-image rec 800 455 restart))
         (hint (place-image (text "HINT" 36  "white") 800 455 p2)))
    (define (placer n)
      (let*((base (place-image (rectangle 1000 20 "solid" "Burlywood") 500 400 hint)))
...
(define (show)
  ...
  (define (cond-move-call i c1 )
    (lambda(x y)
    (if (and ((r i) x y) (not(= i c1))) #t #f)))
  ...
  (define (move-the-disc w x y k)
    (let* ([v (world-stacks w)]
      ...
      (cond [(and (restart-region? x y) (equal? k "button-down")) (begin (set! c1 10) s)]
            ...
            [else (if  (= 10 c1)  (cond [(equal? k "button-down") (cond[((r 0) x y) (begin (set! c1 0) (if (settingc1? c1 v) w w ))]
                                                                       [((r 1) x y) (begin (set! c1 1) (if (settingc1? c1 v) w w ))]
                                                               [((r 2) x y) (begin (set! c1 2) (if (settingc1? c1 v) w w ))]
                                                               [#t w])]
                                        [else w])
                       (cond [(equal? k "button-down")
                              (cond [(and ((r 1) x y) (= 0 c1))
...
;;;;;;;;regions for buttons of hint,restart
...
(define (restart-region? x y)
  (cond [(and (>= x 375) (<= x 625) (>= y 430) (<= y 480)) #t]
        [else #f]))
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;;;region of rod i (where i= 0 1 2)

(define (r i)
  (lambda (x y)
  (cond [(and  (> x (+(* i 250) 225)) (< x (+ (* i 250) 275))
          (> y 90) (< y 390)) #t]
        [else #f])))
...
(define (choose-disc x y)
  (cond ((and (<= x 190) (>= x 110) (<= y 440)) (check (- y 70)))
        ((and (<= x 544) (>= x 456) (<= y 492) (>= y 448)) (set! astate 0) #t)
        (#t #t)))
...
(define (mrender t x y k)
  (if (eq? k "button-down")
      (cond ((and (= 0 astate) (newgame? x y)) (show) (set! astate1 #f) astate)
            ...
            ((= astate 1) (choose-disc x y)  astate)
...
(define (disc-num)
  (let*((bk (rectangle 1000 500 "solid" "cornflowerblue"))
        (bkn (rectangle 80 40 "solid" "forestgreen"))
        (cside (text "CHOOSE THE NUMBER OF DISCS" 36 "black"))
        (nbk (place-image cside 500 30 bk))
        (obk (place-image (place-image (text "OK" 20 "white") 44 22 (rectangle 88 44 "solid" "maroon")) 500 470 nbk))
        (easy (place-image (text "EASY" 20 "white") 40 20 (rectangle 80 40 "solid" "forestgreen")))
        (medium (place-image (text "MEDIUM" 18 "black") 40 20 (rectangle 80 40 "solid" "yellow")))
        (hard (place-image (text "HARD" 20 "white") 40 20 (rectangle 80 40 "solid" "red"))))
  (define (helper c)
    (cond ((> c 10) obk)
          (#t (place-image (getimage c) 150 (+ 70 (+ (* 50 (- c 3)))) (helper (+ c 1))))))
  (define (getimage c)
    (cond [(<= c 5) (place-image (text (number->string c) 20 "white") 40 20 (rectangle 80 40 "solid" "forestgreen"))]
          [(<= c 7) (place-image (text (number->string c) 20 "black") 40 20 (rectangle 80 40 "solid" "yellow"))]
          [(<= c 10) (place-image (text (number->string c) 20 "white") 40 20 (rectangle 80 40 "solid" "Red"))]))
  (let* ((l (helper 3))
         (l1 (place-image easy 850 100 l))
         (l2 (place-image medium 850 150 l1))
         (l3 (place-image hard 850 200 l2)))
    (place-image (rectangle 81 41 "outline" (pen "black" 5 "solid" "round" "bevel")) 150 (+ 70 (+ (* 50 (- n 3)))) l3))))
